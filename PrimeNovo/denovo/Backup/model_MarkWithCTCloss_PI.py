...
class Spec2Pep(pl.LightningModule, ModelMixin):
    ...
    **kwargs : Dict
    ...
    def forward(
        ...
        sequences = [self.decoder.detokenize_truth(t, True) for t in top_tokens]

        Truth = [''.join(sublist) for sublist in sequences]

        if Truth is not None:
            Truth = utils.listify(Truth)
            Truth = [self.decoder.tokenize(s) for s in Truth]
        ...
        def append_to_csv(loss_tensor, scores_tensor, filename='results.csv'):
        ...
        append_to_csv(loss, batchscores, 'results.csv')
